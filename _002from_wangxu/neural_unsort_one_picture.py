...
for epoch in range(1):
	for i, data in enumerate(test_loader,0):
		if 0<= i < 2:#batch_size+2意味着共获得batch_size个图片的hook调用
			img,label= data
...
#dict_sorted_all_output_weight = sorted(dict_all_output_weight.items(),key=lambda item:item[1])
dict_unsorted_all_output_weight = dict_all_output_weight
...
#将sort后的参数存储到文件中
f = open('data_prune/unsorted_record_one_picture.txt','w')
for i  in dict_unsorted_all_output_weight:
	f.write(str(i)+'\n')
f.close()
...
'''for name,parameters in model.named_parameters():
	...
	print(parameters)'''
'''dictToSaveEachLayerOutputWeight()
print('dict_all_output_weight')
print(dict_all_output_weight)
dict_sorted_all_output_weight = sorted(dict_all_output_weight.items(),key=lambda item:item[1])
print('sort to \'dict_all_output_weight\'')
print(dict_sorted_all_output_weight)

#将sort后的参数存储到文件中
f = open('mlp_data/sorted_record.txt','w')
for i  in dict_sorted_all_output_weight:
	f.write(str(i)+'\n')
f.close()'''
